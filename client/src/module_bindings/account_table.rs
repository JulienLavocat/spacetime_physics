...
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.account().name().find(...)`.
pub struct AccountNameUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<Account, String>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> AccountTableHandle<'ctx> {
    /// Get a handle on the `name` unique index on the table `account`.
    pub fn name(&self) -> AccountNameUnique<'ctx> {
        AccountNameUnique {
...
impl<'ctx> AccountNameUnique<'ctx> {
